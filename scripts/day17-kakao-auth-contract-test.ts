...
assert.doesNotMatch(provider, /talk\/memo|sendKakaoMemo|KAKAO_MEMO_ENDPOINT|talk_message/);
...
assert.doesNotMatch(start, /talk_message|notify/);
...
assert.doesNotMatch(callback, /sendKakaoMemo|kakao-token-store|notify/);
...
assert.match(loginPage, /카카오로 로그인하고/);
