...
assert.match(page, /결제와 저장 상태를 확인한 뒤 바로 이어서 보여줄게/);
assert.match(result, /STAGE_COPY/);
assert.match(result, /stageAttempt > 1/);
assert.match(result, /결제는 다시 하지 않아도 돼/);
assert.match(result, /recoverPaymentId=/);
assert.match(result, /현재 응답에 통상적으로 소요되는 시간은 약 5분이야/);
assert.doesNotMatch(result, /완료 · \{elapsedSeconds\}|completedSegments/);

assert.match(narrativeV8, /paid-report-v9-jootopi-direct-voice/);
...
assert.match(narrativeV8, /사용자에게 보이는 모든 문장은 주토피가 친근한 반말/);
