...
assert.match(inputPageSource, /RelationshipNetworkCreateForm/);
assert.match(inputPageSource, /relationship-network\.module\.css/);
assert.match(inputPageSource, /one-to-many-reference-page/);
assert.doesNotMatch(inputPageSource, /className="input-page"|className="input-shell"/);
assert.match(checkoutSource, /redirect\("\/one-to-many"\)/);
assert.doesNotMatch(checkoutSource, /PaymentButton|3,000원/);
...
assert.match(formSource, /1:N 궁합 분석 시작하기 · 3,000원/, "과거 입력 컴포넌트는 저장 주문 호환을 위해 남겨두되 신규 페이지에서 렌더하지 않습니다.");
