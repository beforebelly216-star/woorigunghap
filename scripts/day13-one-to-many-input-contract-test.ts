...
const networkSource = readFileSync("src/components/relationship-network-create-form.tsx", "utf8");
const networkCss = readFileSync("src/app/one-to-many/relationship-network.module.css", "utf8");
...
assert.match(pageSource, /1:N 인연 네트워크/);
assert.match(pageSource, /reference-input-screen/);
assert.match(pageSource, /one-to-many-input-v3\.css/);
assert.match(pageSource, /RelationshipNetworkCreateForm/);
assert.doesNotMatch(pageSource, /OneToManyForm/);
assert.match(networkSource, /내 정보만 입력해 주세요/);
assert.match(networkSource, /\/api\/relationship-networks/);
assert.match(networkSource, /무료 네트워크 만들기/);
assert.match(networkSource, /만 14세 이상/);
assert.match(networkCss, /grid-template-columns:\s*repeat\(3/);
...
console.log("Day 13 legacy input compatibility + free relationship network entry contract checks: PASS");
