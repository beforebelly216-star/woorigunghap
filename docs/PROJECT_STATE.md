...
- 사용자 화면 브랜드명: **우리사주**. 저장소·도메인·DB의 기존 `woorigunghap` 식별자는 호환성을 위해 유지한다.
- 기준 브랜치: `main`
...
- 궁합 scoring **v1.7**: 관계유형별 가중치 + 원점수 순서를 보존하는 대비 보정 + 공개 점수 30~100, 절대 최대 100
...
### 우리사주 브랜드·점수·결제 후 상태 UI — 2026-09-03

- 390px 모바일 셸을 유지하되 전 화면 배경은 순백색 `#FFFFFF`, 본문은 선명한 고대비 색상을 사용한다.
- 기본 글꼴은 별도 웹폰트 다운로드 없이 한국어 시스템 글꼴 스택을 사용해 읽기 속도와 안정성을 높인다.
- 사용자 화면의 좌측 상단 로고는 마스코트 티셔츠의 4색 차트와 `우리사주` 워드마크를 결합한 벡터 로고로 통일한다.
- 결과와 서비스 안내는 특정 캐릭터 이름을 반복하지 않고 친근하고 자연스러운 문체를 사용한다. 전문 용어는 뜻을 먼저 설명하고 한자는 한글 뒤 괄호에 표기한다.
- 버튼·카드·로고에는 짧고 절제된 hover/press 반응을 제공한다.
...
- 공통 마스코트는 `smile / analyzing / idea / thinking / surprised` 고해상도 투명 PNG 표정 세트를 사용한다. 한 화면 안에서 같은 포즈를 겹쳐 쓰지 않고 단계·문맥별로 다르게 배치하며, 캐릭터 이름은 사용자 문구에서 최소화한다.
...
- `주토피의 오늘의 한마디` 기능과 365개 문구 데이터는 전부 삭제했다.
...
- `/free/result`: 원국 → 4개 요약 → 잘 맞는 사주의 구체적인 모습 → 추천 일간 TOP 3 → 마지막 한마디 → 1:1 CTA.
- 추천 일간은 항상 3개이며, 각 카드에 성향 키워드를 풀어쓴 3문장과 천간·지지 선호 조건의 한 줄 설명을 제공한다.

### 1:1 / 무료 1:N 입력

- 기존 desktop-like 입력 UI 폐기, 모바일 레퍼런스형 재구성 완료.
- 출생시간은 오전/오후 선택 없이 **24시간제 HHMM**.
- 제출 후 표시된 필드 오류는 해당 입력을 다시 수정하는 즉시 사라지며, 수정하지 않은 다른 필드 오류는 유지한다.
...
- 1080×1920 스토리 카드와 Shared View에는 관계를 알아볼 수 있도록 참여자 이름을 항상 포함한다. 결과 페이지의 동적 1200×630 OG와 `내 인연 네트워크 만들기` CTA로 공유 유입을 이어간다.
...
- 구조: `01 둘 사이 케미 → 02 두 사람의 기운 → 03 함께 있을 때 살아나는 힘 → 04 관계의 힘이 흐르는 방향 → 05 관계에서 원하는 것 → 06 반복되는 충돌 장면 → 07 지금 가장 중요한 것 → 관계 노트`.
...
- **생성 대기 화면:** 가짜 0~100% 진행률과 단계 카운터 없이 `상세 리포트를 만들고 있어요.`를 한 줄로 표시하고 약 5분이 걸릴 수 있음을 안내한다.
- 로그인 확인 → 결제 확인 → 결과 만들기 상태를 동일한 3단계 카드 UI로 통일하고, 로딩·성공·fatal/config에 같은 레이아웃과 단계 표시를 사용한다.
- 종합점수 옆에 인연 네트워크와 동일한 E 30~49 / D 50~59 / C 60~69 / B 70~79 / A 80~89 / S 90~100 등급을 표시한다.
- 원점수 계산·가중치는 유지하고, 실제 표본에서 중앙 60~70점대로 몰리던 공개 점수만 단조 구간 보정해 약한 궁합은 낮게, 강한 궁합은 A/S까지 도달하도록 대비를 넓혔다.
- 이름 뒤 조사로 문장이 끝나는 비문은 AI 품질 차단 사유로 처리한다. 알려진 `뚜렷하게 이름님과.` 유형은 저장 전과 기존 결과 표시 시 모두 안전하게 교정한다.
- 구형 `60일주 캐릭터` 카드와 원국 아래의 `달빛 항구`, `정원의 설계자` 같은 시적 한줄평은 1:1 결과 렌더 경로에서 제거했다.
- 마지막 공유 영역은 선택 탭 없이 두 사람의 이름이 항상 들어간 `한 장 요약` 카드 하나만 제공한다.
- 히트맵 아래 각 점수는 접이식 `근거 보기`로 계산 근거를 3문장 이내에서 확인한다.
- 1:1 결과의 관계 타이밍 지표와 대운·세운 기반 3년 흐름 섹션은 사용자 화면에서 제거했다. 내부 계산·기존 저장 호환은 유지한다.
- `정보 수준 A/B`와 출생시간 미입력 정보 카드는 사용자 화면에서 사용하지 않는다. 계산은 기존처럼 시주를 제외하고 년·월·일 기준으로 진행한다.
...
- **우리사주 브랜드·점수 대비·결제 후 상태 UI Production validation PASS — 2026-09-03:** implementation source `75c18f7`, deploy source `63de1ec`. 티셔츠 4색 차트+우리사주 벡터 로고, 화면 내 마스코트 포즈 분산, 로그인/결제/결과 공통 3단계 상태 카드, 비문 품질 차단·기존 저장 문구 교정, scoring v1.7 단조 대비 보정, 1:1/1:N 공용 E~S 등급 정책을 적용했다. 관련 contracts 15종, TypeScript, lint(0 errors, 0 warnings), production build(34/34) PASS. Preview `dpl_Cgonh2pcic2h4VdacWAcEUa36jdv` READY 검증 후, 카카오 콜백용 고정 Preview 별칭이 이전 배포를 가리키던 구성을 같은 최신 배포로 갱신했다. 동일 artifact를 Production `dpl_A4tz1Qb93hGfzYKSYYpMcu3rK61P`으로 승격했고, 새 브랜드·로그인 화면·동일 호스트 OAuth callback·1:1 `returnTo`·브라우저 console error 0·최근 30분 Production error 로그 0건을 확인했다.
