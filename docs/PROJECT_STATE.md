...
- 사용자 화면 브랜드명: **주토피**. 저장소·도메인·DB의 기존 `woorigunghap` 식별자는 호환성을 위해 유지한다.
- 기준 브랜치: `main`
...
### 주토피 전면 개편 UI — 2026-09-02

- 390px 모바일 셸을 유지하되 전 화면 배경은 순백색 `#FFFFFF`, 본문은 선명한 고대비 색상을 사용한다.
- 기본 글꼴은 별도 웹폰트 다운로드 없이 한국어 시스템 글꼴 스택을 사용해 읽기 속도와 안정성을 높인다.
- 사용자 화면의 좌측 상단 텍스트 로고는 주토피 캐릭터 기반 로고로 통일한다.
- 결과와 서비스 안내는 주토피가 직접 이야기하는 친근한 반말 문체를 사용한다. 전문 용어는 뜻을 먼저 설명하고 한자는 한글 뒤 괄호에 표기한다.
...
- `/free/result`: 원국 → 4개 요약 → 잘 맞는 사주의 구체적인 모습 → 추천 일간 TOP 3 → 주토피 해설 → 1:1 CTA.
...
- 구조: `01 둘 사이 케미 → 02 두 사람의 기운 → 03 함께 있을 때 살아나는 힘 → 04 관계의 힘이 흐르는 방향 → 05 관계에서 원하는 것 → 06 반복되는 충돌 장면 → 07 지금 가장 중요한 것 → 주토피 마무리`.
