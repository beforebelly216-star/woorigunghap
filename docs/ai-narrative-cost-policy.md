...
- Claude 자체 요청은 한 세그먼트 안에서 사용할 수 없는 응답일 때 최대 1회만 재시도한다.
...
- rate limit, 과부하, 네트워크/서버 timeout, 출력 잘림, JSON 형식 오류는 세그먼트 내부의 제한된 재시도에서 먼저 복구한다. 예산을 소진하면 원인을 분류해 종료하고, 사용자는 이미 완료·저장된 세그먼트를 유지한 채 같은 결제로 다시 시도할 수 있다.
