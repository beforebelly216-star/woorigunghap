...
export default function AccountReportsPage() {
  ...
  async function deleteReport(report: ReportSummary) {
    const confirmed = window.confirm("이 결과를 삭제하면 복구할 수 없어. 상세 리포트와 입력정보는 삭제되고, 결제 거래기록은 법정 보존 의무에 필요한 최소 정보만 남아. 삭제할까?");
    if (!confirmed) return;
    ...
    try {
      ...
      setDeleteMessage("네트워크 상태를 확인한 뒤 다시 시도해 줘.");
    } finally {
  ...
  return <main className="library-page">
    <section className="library-shell">
      ...
      <h1>내 궁합 보관함</h1>
      <p className="library-intro">구매 리포트와 이 브라우저에서 만든 무료 인연 네트워크를 여기서 다시 확인할 수 있어.</p>
      ...
      <div aria-live="polite" aria-busy={state.status === "loading"}>
        ...
        {state.status === "guest" ? <div className="library-state">
          <h2>로그인이 필요해요</h2>
          <p>카카오 로그인 뒤 저장한 구매 리포트와 무료 인연 네트워크를 다시 볼 수 있어.</p>
        ...
        {state.status === "failed" ? <div className="library-state" role="alert">
          ...
          <p>기존 결과는 사라지지 않았어. 네트워크 상태를 확인한 뒤 다시 불러와 줘.</p>
        ...
        {state.status === "ready" && state.reports.length === 0 ? <div className="library-state">
          ...
          <p>결제가 확인되면 결과를 만드는 중부터 표시돼. 무료 인연 네트워크는 만들자마자 위 목록에 저장돼.</p>
