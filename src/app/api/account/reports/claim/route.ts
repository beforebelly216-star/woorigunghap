...
import { loadServerReportForAccess } from "@/lib/server-report-store";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const claimed = await claimAccountReport(user.userId, paymentId, recovered.order.product);
    if (claimed === "conflict") {
    ...
    return NextResponse.json({ claimed: true, product: recovered.order.product }, { headers: privateHeaders });
  } catch (error) {
