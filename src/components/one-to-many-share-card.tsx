...
const CARD_OPTIONS: Record<GrowthSharePurpose, CardOption> = {
  receipt: { purpose: "receipt", label: "관계 영수증", eyebrow: "우리 관계 영수증" },
  recap: { purpose: "recap", label: "한 장 요약", eyebrow: "우리 관계 한 장 요약" },
  relationship_label: { purpose: "relationship_label", label: "관계 한 줄", eyebrow: "우리 관계를 한 줄로" },
  two_sides: { purpose: "two_sides", label: "강한 축 · 조율", eyebrow: "강한 지점과 맞출 지점" },
  send_this: { purpose: "send_this", label: "이거 보내기", eyebrow: "친구에게 이 결과 보내기" },
};
...
async function createShareImageBlob(input: { relationshipLabel: string; candidateCount: number; purpose: GrowthSharePurpose; eyebrow: string; shareCopy: string; roles: RoleHighlight[]; sides: SideHighlight }) {
  ...
  if (input.purpose === "receipt") {
    input.roles.slice(0, 3).forEach((role, index) => {
      ...
      ctx.fillStyle = "#222026"; ctx.font = "900 36px Pretendard, sans-serif"; ctx.fillText(`${role.displayName} · ${role.score}점`, 200, y + 102);
    });
    ...
    input.roles.slice(0, 3).forEach((role, index) => {
      ...
      ctx.fillStyle = "#222026"; ctx.font = "900 34px Pretendard, sans-serif"; ctx.fillText(role.displayName, 200, y + 92);
    });
    ...
    input.roles.slice(0, 3).forEach((role, index) => {
      ...
      ctx.fillStyle = "#222026"; ctx.font = "900 37px Pretendard, sans-serif"; ctx.fillText(role.displayName, 200, y + 108);
    });
  }
...
export function OneToManyShareCard({ view }: OneToManyShareCardProps) {
  ...
  const [purpose, setPurpose] = useState<GrowthSharePurpose>(() => initialPurpose); const [shareState, setShareState] = useState<"idle" | "shared" | "copied" | "saved" | "failed">("idle");
  ...
  async function saveImage() {
    try { const blob = await createShareImageBlob({ relationshipLabel: view.relationshipLabel, candidateCount: view.rankings.length, purpose, eyebrow: selectedOption.eyebrow, shareCopy, roles, sides }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `woorisaju-comparison-${purpose}.png`; link.click(); trackGrowthEvent({ eventName: "share_image_download", product: "oneToMany", relationshipType, surface: "one_to_many_share_card", sharePurpose: purpose, experimentArm }); window.setTimeout(() => URL.revokeObjectURL(url), 1_000); setShareState("saved"); } catch { setShareState("failed"); }
  }
