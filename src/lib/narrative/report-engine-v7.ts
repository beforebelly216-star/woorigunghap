...
async function generateDynamics(apiKey: string, model: string, payloadText: string, relationshipRules: string) {
  return requestStructuredSegment<DynamicsSegment>({
    ...
    system: `${BASE_RULES}\n\n${relationshipRules}\n\n[담당 범위: CH2 상대 해부 + CH3 나의 강점 + 기본 케미]\n- 이 세그먼트 전체는 공백 제외 약 1,800~2,600자로 간결하게 작성하고 같은 뜻을 반복하지 마세요.\n- chemistry.overview는 2문장, dayMaster/dayBranch/yinYang/elements는 각각 1문장으로 계산 의미와 현실에서 확인할 장면을 연결하세요.\n- bondAndFriction.overview는 2문장. positiveInteractions와 frictionInteractions는 실제 evidence가 있는 것만 최소 2개씩 우선 작성하고 각 항목을 충분히 풀이하세요.\n- realLifeManifestations는 2개이며 연락, 약속, 감정표현, 의사결정처럼 이 관계 유형에서 실제로 관찰할 장면으로 쓰세요.\n- directionalImpact의 overview/aToB/bToA/beneficialSupply/burdenSupply/asymmetry는 각각 1~2문장. {{SELF}}→{{PARTNER}}와 {{PARTNER}}→{{SELF}}를 반드시 구분하고 같은 문장을 뒤집어 쓰지 마세요.\n- 관계 역할 맞물림 점수는 관계에서 역할이 어느 정도 상호 보완되는지에 대한 요약값일 뿐입니다. 이 값에서 보살핌 욕구, 존재감, 사랑 방식 같은 숨은 심리를 추론하지 마세요.\n- partnerDeepDive.outerInnerContrast는 2~3문장. '겉과 속이 실제로 다르다'고 단정하지 말고, 상황에 따라 다르게 보일 수 있는 관찰 가능한 반응 차이를 설명하세요.\n- partnerDeepDive.comfortTriggers / sensitiveTriggers / preferredInteraction은 각각 2개. 항목마다 '어떤 상황에서 → 어떤 반응이 관찰될 수 있는지 → 어떻게 확인·배려할지'가 읽히게 쓰세요.\n- partnerDeepDive.observableScenes는 2개. situation, likelyReaction, considerateResponse를 모두 구체적으로 쓰고 likelyReaction은 가능성 표현을 사용하세요.\n- partnerDeepDive.profileTags는 4~6개, 짧지만 이 조합의 검증된 근거와 일치하는 표현만 쓰세요.\n- personalLeverage.topStrengths는 정확히 3개를 우선하세요. whyItWorks와 howToUse를 각각 1~2문장으로 써서 '왜 이 상대에게 통할 가능성이 있는지'가 분명해야 합니다.\n- conversationScripts는 2개. 실제로 말할 수 있는 짧은 문장과 피해야 할 말투를 함께 제시하세요. 조종·압박 문구는 금지합니다.\n- backfireHabits는 2개. 내 강점을 과하게 썼을 때 관찰될 수 있는 역효과와 교정 행동을 한 쌍으로 작성하세요.`,
