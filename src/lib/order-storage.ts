...
function parseOrderDraft(raw: string | null, paymentId: string): OrderDraft | null {
  if (!raw) return null;
  ...
  try {
    ...
    if (
      ...
      (parsed.product !== "oneToOne" && parsed.product !== "oneToMany") ||
...
export function loadOrderDraft(paymentId: string): OrderDraft | null {
