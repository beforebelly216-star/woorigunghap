...
const STEM_TRAITS: Record<string, { keywords: string[]; strength: string; complement: string }> = {
  갑: { keywords: ["곧은 추진력", "성장 지향", "책임감"], strength: "방향을 잡으면 꾸준히 밀고 가며 관계에서도 기준과 책임을 분명히 하는 힘이 있어.", complement: "속도를 부드럽게 조율하고 감정을 세밀하게 읽는 기운이 더해지면 관계의 폭이 넓어져." },
  을: { keywords: ["유연함", "관찰력", "관계 감각"], strength: "상황을 읽고 유연하게 움직이며 상대의 반응을 섬세하게 살피는 힘이 있어.", complement: "결정을 미루지 않도록 방향을 잡고 안정적으로 받쳐 주는 기운이 더해지면 편안해." },
  병: { keywords: ["표현력", "활력", "직진성"], strength: "감정과 의사를 비교적 분명하게 드러내고 관계에 활력을 넣는 힘이 있어.", complement: "과열될 때 속도를 낮추고 차분하게 정리하는 기운이 더해지면 오래 가기 쉬워." },
  정: { keywords: ["세심함", "온기", "집중력"], strength: "작은 신호를 잘 읽고 가까운 사람에게 꾸준히 온기를 전하는 힘이 있어.", complement: "혼자 오래 고민하지 않도록 안정적인 반응과 분명한 확신을 주는 기운이 도움이 돼." },
  무: { keywords: ["안정감", "책임감", "버팀목"], strength: "관계의 중심을 잡고 쉽게 흔들리지 않으며 약속을 지키는 힘이 커.", complement: "고정된 방식에 머물지 않도록 유연함과 새로운 자극을 주는 기운이 더해지면 좋아." },
  기: { keywords: ["배려", "현실감", "돌봄"], strength: "상대의 필요를 실용적으로 챙기고 관계를 세심하게 관리하는 힘이 있어.", complement: "과도하게 맞추지 않도록 자기표현을 북돋고 방향을 선명하게 잡아 주는 기운이 좋아." },
  경: { keywords: ["결단력", "원칙", "실행력"], strength: "문제를 분명하게 보고 빠르게 정리하며 관계에서도 기준을 세우는 힘이 있어.", complement: "말의 강도를 부드럽게 낮추고 감정의 여백을 만드는 기운이 더해지면 균형이 좋아져." },
  신: { keywords: ["정교함", "기준", "섬세함"], strength: "관계의 작은 차이를 잘 알아차리고 완성도와 신뢰를 중요하게 여기는 힘이 있어.", complement: "완벽함보다 과정의 편안함을 느끼게 하는 따뜻하고 여유로운 기운이 도움이 돼." },
  임: { keywords: ["포용력", "적응력", "확장성"], strength: "상황 변화에 유연하고 상대의 여러 면을 받아들이며 관계의 가능성을 넓히는 힘이 있어.", complement: "흐름이 너무 넓어지지 않도록 현실적인 기준과 일관성을 잡는 기운이 좋아." },
  계: { keywords: ["공감력", "직관", "세밀함"], strength: "작은 감정 변화와 분위기를 빠르게 읽고 깊이 공감하는 힘이 있어.", complement: "생각이 안으로만 흐르지 않도록 표현을 끌어내고 안정감을 주는 기운이 더해지면 편안해." },
};
...
function relationLabel(relation: ReturnType<typeof scoreDayMasterCompatibility>) {
  ...
  if (relation.relation === "GENERATES" && relation.direction === "A_TO_B") return "내 활력이 상대의 성장을 돕는 관계";
...
function recommendationReason(
  ...
  if (lowElements.includes(candidate.element)) reasons.push(`내 원국에서 상대적으로 적은 ${ELEMENT_LABEL[candidate.element]} 기운을 보완하는 방향이야.`);
  if (relation.polarityRelation === "OPPOSITE") reasons.push("음양이 달라 서로 다른 방식으로 역할을 나누기 쉬운 조합이야.");
  else reasons.push("비슷한 음양 리듬을 공유해 행동 속도와 반응 방식이 익숙하게 느껴질 수 있어.");
  if (weights[candidate.element] === Math.min(...Object.values(weights))) reasons.push(`특히 ${ELEMENT_LABEL[candidate.element]}이 원국에서 가장 적게 드러나 보완 우선도가 높아.`);
  return reasons;
}

function relationshipPattern(stem: string) {
  const copy: Record<string, string> = {
    갑: "관계의 방향을 선명하게 잡고 함께 성장하는 쪽으로 움직이기 쉬운 타입이야.",
    을: "서로의 기분과 상황을 세밀하게 살피며 부드럽게 맞춰 가기 쉬운 타입이야.",
    병: "표현과 행동이 빠르고 관계에 활력을 넣어 정체된 분위기를 깨기 쉬운 타입이야.",
    정: "감정을 섬세하게 확인하고 작은 배려를 꾸준히 쌓기 쉬운 타입이야.",
    무: "약속과 생활 리듬을 안정적으로 잡아 오래 가는 기반을 만들기 쉬운 타입이야.",
    기: "현실적인 돌봄과 세심한 배려로 일상의 편안함을 만들기 쉬운 타입이야.",
    경: "문제를 미루기보다 분명하게 정리하고 서로의 기준을 세우기 쉬운 타입이야.",
    신: "세밀한 신뢰와 약속을 중요하게 여겨 관계의 완성도를 높이기 쉬운 타입이야.",
    임: "상대의 여러 면을 받아들이고 새로운 경험을 함께 넓히기 쉬운 타입이야.",
    계: "감정의 미세한 변화를 빠르게 알아차리고 깊은 공감으로 가까워지기 쉬운 타입이야.",
  };
  return copy[stem] ?? "서로 다른 기운을 보완하며 관계의 균형을 만들기 쉬운 타입이야.";
}
...
function cautionCopies(userStem: string, userBranch: string, weights: Record<FiveElement, number>, rankedStems: ReturnType<typeof rankStems>, rankedBranches: ReturnType<typeof rankBranches>) {
  ...
  const copies = [`이미 비교적 많이 드러나는 ${ELEMENT_LABEL[maxElement]} 기운이 상대 원국에서도 지나치게 강하면 비슷한 패턴이 과해질 수 있어.`];
  if (weakStem) copies.push(`${weakStem.candidate.stem}(${weakStem.candidate.hanja}) 일간은 ${userStem} 일간과 극 관계가 먼저 작동해, 다른 보완 요소가 적으면 긴장감이 커질 수 있어.`);
  if (clashBranch) copies.push(`${clashBranch.candidate.branch}(${clashBranch.candidate.hanja}) 지지가 강하게 작동하면 내 일지 ${userBranch}와 충이 생겨 생활 리듬이나 감정 반응이 자주 엇갈릴 수 있어.`);
...
export function calculateSoulmateResult(person: PersonBirthInput): SoulmateResult {
  ...
  const topStems = rankedStems.slice(0, 3);
  ...
  const recommendations = topStems.map((item, index) => ({
    ...
    personalitySummary: [
      `${item.candidate.stem}(${item.candidate.hanja}) 일간은 ${STEM_TRAITS[item.candidate.stem].keywords.join("·")}의 결이 또렷한 사람이야.`,
      STEM_TRAITS[item.candidate.stem].strength,
      `${relationshipPattern(item.candidate.stem)} 그래서 네 앞에서는 장점을 비교적 편하게 드러낼 수 있어.`,
    ],
  ...
  return {
    ...
    self: {
      ...
      complement: `${traits.complement} 현재 원국의 오행 분포에서는 ${preferredElementLabels.join("·")} 기운을 보완 관점에서 함께 살펴볼 만해.`,
    },
    elementBalance,
    ...
    detailed: {
      ...
      methodNote: "추천 일간은 만세력 원국을 바탕으로 일간의 생극 관계, 오행 분포, 음양의 균형, 일지의 합·충 관계를 함께 고려하여 산출합니다.",
    },
    zootopi: {
      ...
      closing: `천생연분은 일간 이름 하나로 확정되는 사람이 아니야. 그래도 네 사주에서 어떤 기운과 구조가 편안하게 맞물리는지는 꽤 구체적으로 좁혀 볼 수 있어. 실제로 마음에 둔 사람이 있다면 두 사람의 원국을 함께 놓고 1:1 궁합으로 확인해 봐.`,
    },
  };
}
